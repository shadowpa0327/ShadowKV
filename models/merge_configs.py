...
class LayerGroup:
    ...
    depending on 'layer_merge_impl' in PaluConfig.

    After PaluConfig.__post_init__ runs, the 'irrelevant' fields
...
class PaluConfig:
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "PaluConfig":
        """
...
def generate_consecutive_layer_groups(
    ...
    They will be filled (or remain None) during PaluConfig.__post_init__.
    """
    groups = []
    current = start_layer
...
def generate_consecutive_palu_config(
    ...
    """
    Quickly build a PaluConfig with consecutive-layer groups.
    ...
    Returns:
      A fully constructed PaluConfig with consecutive groups,
...
if __name__ == "__main__":
    ...
    # loaded_svd = PaluConfig.from_yaml(svd_yaml)
    ...
    # loaded_slerp = PaluConfig.from_yaml(slerp_yaml)
